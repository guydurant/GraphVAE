...
class GraphVAE(nn.Module):
    def __init__(self, input_dim, hidden_dim, latent_dim, max_num_nodes, pool='sum'):
        ...
        self.conv1 = model.GraphConv(
            input_dim=input_dim, output_dim=hidden_dim)
        self.bn1 = nn.BatchNorm1d(input_dim)
        self.conv2 = model.GraphConv(
        ...
        self.vae = model.MLP_VAE_plain(
            input_dim * input_dim, latent_dim, output_dim)
    ...
    def recover_adj_lower(self, l):
        ...
        adj[torch.triu(torch.ones(self.max_num_nodes,
                                  self.max_num_nodes)) == 1] = l
        return adj
    ...
    def forward(self, input_features, adj):
        ...
        init_assignment = torch.ones(
            self.max_num_nodes, self.max_num_nodes) * init_corr
        ...
        adj_vectorized = adj_permuted[torch.triu(torch.ones(
            self.max_num_nodes, self.max_num_nodes)) == 1].squeeze_()
